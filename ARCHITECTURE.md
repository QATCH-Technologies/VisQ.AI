...
```mermaid
flowchart TD
    A["Raw formulation export<br/>Dropbox, data/latest/*.xlsx or *.csv"]
    B["visqai.paths.load_table<br/>xlsx/csv, whichever it is"]
    C["visqai.features.dataprocessor.build_feature_frame<br/>categorical/charge featurization, physics priors, engineered columns"]
    D["visqai.training.data.load_and_preprocess<br/>fits ColumnTransformer + physics_scaler<br/>PCHIP-resamples each curve onto a fixed shear grid"]
    E["visqai.training.run.train_final_model<br/>optionally preceded by Optuna tuning (training.tuning)<br/>trains a models.cnp.CrossSampleCNP"]
    F["Checkpoint saved<br/>MODELS_ROOT/checkpoints/&lt;date&gt;/&lt;time&gt;/best_model.pth<br/>+ preprocessor.pkl, physics_scaler.pkl, protected_indices.pkl"]
    G["visqai.eval.*_eval<br/>sanity-check the checkpoint"]
    H["visqai.inference.predictor.ViscosityPredictorCNP<br/>serve predictions"]
    I["visqai.packaging.packager.SecurePredictorPackager<br/>ship to a client"]
    J["MODELS_ROOT/production/&lt;date&gt;/&lt;time&gt;/visqai_*.visq<br/>signed zip"]
...
resolve dynamically per machine - `VISQAI_DATA_ROOT`/`VISQAI_MODELS_ROOT` env vars if set, else
Dropbox's own `info.json` (correct even with a customized install location), else a
...
- **`categorical.py`** - replaces one-hot categorical encodings (buffer/salt/stabilizer/
  surfactant/excipient/protein-class type) with physicochemical property vectors, so the model
  can interpolate between chemically similar entities instead of treating every category as an
  orthogonal label.
...
- `correction_head(query, r) = g` - the only path with access to context. Its final layer is
  zero-initialized, so at construction (and for any `r` it hasn't learned to react to) `g`
  contributes exactly zero: `prediction = prior_head(query) + g(query, r=0) = prior_head(query)`.
...
worse than zero-shot on any held-out group, or the run fails loudly rather than shipping a
regression.
...
the specific case where a few real context points are available. Descriptor-OOD down-weighting
...
[CONTRIBUTING.md](CONTRIBUTING.md#adding-a-new-evaluation-suite)):
...
are always auto-generated (see [Storage](#storage-checkpoints-and-production-packages) below) -
never hand-picked.
...
Nothing under `models/` lives in the repo (proprietary). Both checkpoints and `.visq` packages
are written to the team's Dropbox under an **auto-generated `<date>/<time>` directory**
