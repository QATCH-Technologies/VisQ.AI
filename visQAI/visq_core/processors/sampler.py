...
try:
    ...
    from visq_core.managers.asset_manager import AssetError, AssetManager
    from visq_core.models.formulation import Formulation, ViscosityProfile
    from visq_core.models.ingredient import Ingredient
    ...
    from QATCH.VisQAI.visq_core.controller.formulation_controller import (
        FormulationController,
    )
    from QATCH.VisQAI.visq_core.controller.ingredient_controller import (
        IngredientController,
    )
    from QATCH.VisQAI.visq_core.db.db import Database
    from QATCH.VisQAI.visq_core.managers.asset_manager import AssetError, AssetManager
    from QATCH.VisQAI.visq_core.models.formulation import Formulation, ViscosityProfile
    from QATCH.VisQAI.visq_core.models.ingredient import Ingredient
        ...
        for _ in range(n):
            ...
            for (low, high), enc in zip(self._bounds, self._encoding):
                ...
                if enc["type"] == "cat":
                    ...
                    if not choices:
                        raise RuntimeError(f"No choices for categorical feature {feat}")
                    suggestions[feat] = np.random.choice(choices)
                else:
                    ing_type = suggestions.get(feat.replace("_conc", "_type"), None)
    ...
    def _build_formulation(
        ...
        if val := suggestions.get("Protein_type"):
            prot = (
                ...
                else self.ing_ctrl.get_protein_by_name(val)
            )
        ...
        if val := suggestions.get("Buffer_type"):
            buff = (
                val
                if isinstance(val, Ingredient)
                else self.ing_ctrl.get_buffer_by_name(val)
            )
        ...
        if val := suggestions.get("Salt_type"):
            salt = (
                val
                if isinstance(val, Ingredient)
                else self.ing_ctrl.get_salt_by_name(val)
            )
        ...
        if val := suggestions.get("Stabilizer_type"):
            stab = (
                val
                if isinstance(val, Ingredient)
                else self.ing_ctrl.get_stabilizer_by_name(val)
            )
            form.set_stabilizer(
        ...
        if val := suggestions.get("Surfactant_type"):
            surf = (
                val
                if isinstance(val, Ingredient)
                else self.ing_ctrl.get_surfactant_by_name(val)
            )
            form.set_surfactant(
        ...
        if val := suggestions.get("Excipient_type"):
            excip = (
                val
                if isinstance(val, Ingredient)
                else self.ing_ctrl.get_excipient_by_name(val)
            )
            form.set_excipient(
