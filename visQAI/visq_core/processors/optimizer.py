...
class Optimizer:
    ...
    def _decode(self, x: np.ndarray) -> Dict[str, Any]:
        ...
        for xi, enc in zip(x, self.encoding):
            ...
            if enc["type"] == "cat":
                ...
                idx = int(np.clip(round(xi), 0, len(self.cat_choices[feat]) - 1))
    ...
    def _build_formulation(self, feat_dict: Dict[str, Any]) -> Formulation:
        ...
        form.set_temperature(temp=feat_dict["Temperature"])
        return form
    ...
    def _objective(self, x: np.ndarray) -> float:
        ...
        pred = self.predictor.predict(df=formulation.to_dataframe(training=False))
