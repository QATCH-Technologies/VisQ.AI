                ...
                if n == 0 or n == 2:
                    ...
                    if isinstance(item, dict):
                        ...
                        if m not in [
                            ...
                            if not any(
                                str(c).casefold() == "none" for c in local_choices
                            ):
                        ...
                        if len(selected):
                            try:
                                newitem.setCurrentIndex(
                                    [
                                        newitem.itemText(i).casefold()
                                        for i in range(newitem.count())
                                    ].index(str(selected).casefold())
                                )  # case-insensitive matching
                            except ValueError:
                                print(
                                    f'WARNING: Entry "{str(selected)}" is not a known type!'
                                )
    ...
    def allSet(self) -> bool:
        for n, key in enumerate(self.data.keys()):
            for m, _ in enumerate(self.data[key]):
                ...
                if item.background().color().name() in [
                    Color.light_yellow.name(),
                    Color.light_red.name(),
                ]:
                    return False
        return True
            ...
            try:
                if class_item is not None:
                    class_item.setCurrentIndex(
                        [
                            class_item.itemText(i).casefold()
                            for i in range(class_item.count())
                        ].index(protein_class.casefold())
                    )  # case-insensitive matching
            except ValueError:
                print(
                    f'WARNING: Entry "{protein_class}" is not a known Protein Class! Using "Other" instead.'
                )
                try:
                    class_item.setCurrentIndex(
                        [
