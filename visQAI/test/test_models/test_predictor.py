...
class TestLoadSourceModules(unittest.TestCase):
    ...
    def test_load_source_modules_priority_order(self):
        ...
        if "config" in loaded_modules and "inference" in loaded_modules:
            self.assertLess(
                loaded_modules.index("config"), loaded_modules.index("inference")
            )
...
class TestPredictMethods(unittest.TestCase):
    ...
    def test_predict_with_loaded_predictor(self):
        ...
        mock_predictor.predict.assert_called_once_with(
            self.test_df, return_uncertainty=False
        )
...
class TestLearnMethod(unittest.TestCase):
    ...
    def test_learn_with_default_parameters(self):
        ...
        mock_predictor.learn.assert_called_once_with(
            self.test_df, n_epochs=None, verbose=True
        )
    ...
    def test_learn_with_custom_parameters(self):
        ...
        mock_predictor.learn.assert_called_once_with(
            self.test_df, n_epochs=20, verbose=False
        )
        self.assertEqual(result, expected_result)
...
def suite():
    ...
    test_suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestPredictorInitialization)
    )
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestLoadZip))
    test_suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestLoadSourceModules)
    )
    ...
    test_suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestPredictorIntegration)
    )
