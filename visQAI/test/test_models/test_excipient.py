...
class TestBaseExcipient(unittest.TestCase):
    def test_equality_and_hash(self):
        uid = uuid.uuid4()
        e1 = BaseExcipient(name="Test", id=uid)
        e2 = BaseExcipient(name="Other", id=uid)
        self.assertEqual(e1, e2)
        self.assertEqual(hash(e1), hash(e2))
        e3 = BaseExcipient(name="Test", id=uuid.uuid4())
        self.assertNotEqual(e1, e3)

    def test_not_equal_to_other_types(self):
        e = BaseExcipient(name="X")
        self.assertFalse(e == "X")
        self.assertNotEqual(e, 123)

    def test_name_setter_valid(self):
        e = BaseExcipient(name="Orig")
        e.name = " NewName "
        self.assertEqual(e.name, "NewName")

    def test_name_setter_invalid(self):
        e = BaseExcipient(name="Test")
...
class TestExcipientProfile(unittest.TestCase):
    ...
    def test_duplicate_variation_raises(self):
        self.profile.add_variation(
            etype="Buffer", concentration=1.0, unit=ConcentrationUnit.MILLIGRAM_PER_ML
        )
        with self.assertRaises(ValueError):
            self.profile.add_variation(
                etype="Buffer", concentration=1.0, unit=ConcentrationUnit.MILLIGRAM_PER_ML
    ...
    def test_distinct_units_allowed(self):
        self.profile.add_variation(
            etype="Buffer", concentration=1.0, unit=ConcentrationUnit.PERCENT_W_V
        )
        self.profile.add_variation(
            etype="Buffer", concentration=1.0, unit=ConcentrationUnit.PERCENT_V_V
        )
        self.assertEqual(len(self.profile.get_variations()), 2)

    def test_variations_return_copy(self):
        self.profile.add_variation(
            etype="Salt", concentration=0.2, unit=ConcentrationUnit.MILLIMOLAR
        )
    ...
    def test_base_excipient_setter_valid(self):
        new_base = BaseExcipient(name="NewBase")
