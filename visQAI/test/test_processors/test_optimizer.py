...
from visq_core.managers.asset_manager import AssetManager
...
from visq_core.processors.optimizer import Constraints, Optimizer
...
class TestConstraints(unittest.TestCase):
    ...
    def test_add_choices_invalid_feature(self):
        dummy = Protein(
            enc_id=-1, name="P", molecular_weight=50.0, pI_mean=6.0, pI_range=0.2
        )
        with self.assertRaises(ValueError) as cm:
    ...
    def test_add_choices_not_persisted(self):
        prot = Protein(
            enc_id=-1, name="NewProt", molecular_weight=30.0, pI_mean=5.0, pI_range=0.1
        )
        with self.assertRaises(ValueError) as cm:
    ...
    def test_add_choices_valid(self):
        prot = Protein(
            enc_id=-1, name="GoodProt", molecular_weight=25.0, pI_mean=4.5, pI_range=0.2
        )
    ...
    def test_build_with_defaults_and_ranges(self):
        ing_list = [
            Protein(
                enc_id=-1, name="P", molecular_weight=10.0, pI_mean=6.0, pI_range=0.2
            ),
...
class TestOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.predictor = Predictor(
            zip_path=os.path.join("test", "assets", "VisQAI-base.zip")
        )
