...
class TestMetrics(unittest.TestCase):
    ...
    def test_smape(self):
        ...
        zero_df = pd.DataFrame({"actual": [0.0, 0.0], "predicted": [0.0, 0.0]})
    ...
    def test_relative_bias(self):
        ...
        # Test with zero mean
        zero_mean_df = pd.DataFrame({"actual": [0.0, 0.0], "residual": [1.0, -1.0]})
    ...
    def test_theil_u(self):
        ...
        single_df = pd.DataFrame({"actual": [10.0], "predicted": [12.0]})
    ...
    def test_cv_rmse(self):
        ...
        zero_mean_df = pd.DataFrame(
            {"actual": [0.0, 0.0], "predicted": [1.0, -1.0], "residual": [1.0, -1.0]}
        )
    ...
    def test_log_accuracy_ratio(self):
        ...
        invalid_df = pd.DataFrame({"actual": [0.0, 1.0], "predicted": [1.0, 2.0]})
    ...
    def test_shapiro_wilk_p(self):
        ...
        small_df = pd.DataFrame({"residual": [1.0, 2.0]})
    ...
    def test_anderson_darling_stat(self):
        ...
        # Test with insufficient data
        small_df = pd.DataFrame({"residual": [1.0]})
    ...
    def test_compute_overall(self):
        ...
        results = self.metrics.compute_overall(self.simple_df, metrics=metrics_list)
    ...
    def test_compute_overall_with_confidence_intervals(self):
        ...
        results = self.metrics.compute_overall(
            self.simple_df, metrics=metrics_list, include_confidence_intervals=True
        )
    ...
    def test_compute_overall_with_invalid_metric(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            results = self.metrics.compute_overall(
                self.simple_df, metrics=["invalid_metric"]
            )
    ...
    def test_evaluate_metric_status(self):
        ...
        # Test unknown metric
        status_unknown = self.metrics._evaluate_metric_status("unknown_metric", 0.5)
    ...
    def test_add_metric(self):
        ...
        result = self.metrics.compute_overall(self.simple_df, metrics=["custom_sum"])
    ...
    def test_bootstrap_ci(self):
        ...
        ci_low, ci_high = self.metrics._bootstrap_ci(
            self.simple_df, "mae", n_bootstrap=100
        )
    ...
    def test_bootstrap_ci_with_failing_metric(self):
        ...
        ci_low, ci_high = self.metrics._bootstrap_ci(
            self.simple_df, "failing_metric", n_bootstrap=10
        )
    ...
    def test_all_metrics_callable(self):
        ...
        for metric_name, metric_func in self.metrics.metrics.items():
            self.assertTrue(
                callable(metric_func), f"Metric '{metric_name}' is not callable"
            )
    ...
    def test_metrics_return_numeric_values(self):
        ...
        for metric_name in self.metrics.get_available_metrics():
            try:
                ...
                self.assertTrue(
                    isinstance(result, (int, float, np.number)) or np.isnan(result),
