...
class BaseTestDatabase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self.db.close()
        self.assertTrue(
            self.db_file.exists(), "Database file should exist after close."
        )
        if self.db_file.exists():
            self.db_file.unlink()
    ...
    def test_add_and_get_each_ingredient_type(self):
        ...
        self.assertIsInstance(self.db.get_ingredient(ids["Stabilizer"]), Stabilizer)
        self.assertIsInstance(self.db.get_ingredient(ids["Surfactant"]), Surfactant)
        self.assertIsInstance(self.db.get_ingredient(ids["Salt"]), Salt)
        self.assertIsInstance(self.db.get_ingredient(ids["Excipient"]), Excipient)
    ...
    def test_get_all_and_delete_ingredient(self):
        ...
        self.assertTrue(
            self.db.delete_ingredient(id1), "Deletion by ID should return True"
        )

        remaining = self.db.get_all_ingredients()
        self.assertEqual(
            len(remaining), 1, "One ingredient should remain after deletion"
        )

        rem_p = [i for i in remaining if isinstance(i, Protein)]
        self.assertEqual(len(rem_p), 1, "Should have exactly one Protein remaining")
        self.assertIn(
            rem_p[0].class_type,
            ProteinClass.all(),
            "Remaining protein should have valid class_type",
        )
        self.assertEqual(rem_p[0].enc_id, 2, "Remaining protein should be Y (enc_id=2)")
    ...
    def test_update_ingredient(self):
        ...
        self.assertTrue(
            self.db.update_ingredient(iid, newb), "Expected update to return True"
        )
        ...
        self.assertFalse(
            self.db.update_ingredient(9999, newb),
    ...
    def _make_formulation(self):
        ...
        p = Protein(
            ...
            pI_mean=6,
            pI_range=0.2,
            class_type=ProteinClass.FC_FUSION,
        )
    ...
    def test_get_all_and_delete_formulation(self):
        ...
        self.assertTrue(
            self.db.delete_formulation(fid),
            "Expected delete_formulation to return True",
        )
        self.assertEqual(
            len(self.db.get_all_formulations()), 1, "One formulation should remain"
        )

        self.db.delete_all_formulations()
        self.assertEqual(
            self.db.get_all_formulations(),
            [],
    ...
    def test_components_linked_to_existing_ingredients(self):
        ...
        expected_types = [k for k, v in form._components.items() if v is not None]
        self.assertEqual(
        ...
        for comp_type, iid in comp_rows:
            ...
            self.assertEqual(
                name_db, orig_ing.name, f"Name mismatch for component '{comp_type}'"
            )
    ...
    def test_database_file_reload(self):
        ...
        self.db.close()
        self.assertTrue(self.db_file.exists(), "Database file should exist after close")
        self.db = Database(self.db_file, self.encryption_key, self.parse_file_key)

        got = self.db.get_formulation(fid)
        self.assertIsNotNone(got, "Expected to retrieve a Formulation after reload")
        self.assertAlmostEqual(got.temperature, 37.0)
    ...
    def test_database_file_write_on_close_if_missing(self):
        ...
        self.db.close()
        self.db_file.unlink()
        self.assertFalse(self.db_file.exists(), "Database file should be removed")

        # Opening without making changes and then closing should recreate the file
        self.db = Database(self.db_file, self.encryption_key, self.parse_file_key)
        self.assertEqual(
            self.db.conn.total_changes, self.db.init_changes, "No changes yet"
        )
        self.db.close()
        self.assertTrue(
            self.db_file.exists(), "Database file should be recreated on close"
        )
        ...
        self.assertGreater(
            self.db.conn.total_changes,
            self.db.init_changes,
    ...
    def test_database_file_cannot_unlink_while_open(self):
        ...
        self.assertTrue(self.db_file.exists(), "Database file should still exist")
        self.assertTrue(
            permission_error, "Expected PermissionError when deleting open DB file"
        )
