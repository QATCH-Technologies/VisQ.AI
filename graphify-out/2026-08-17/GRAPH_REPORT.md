# Graph Report - VisQ.AI  (2026-08-14)

## Corpus Check
- 97 files · ~105,969 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 961 nodes · 1701 edges · 56 communities (47 shown, 9 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 352 edges (avg confidence: 0.79)
...
3. `fit_fv_regime_params()` - 19 edges
4. `build_gap_ranking()` - 17 edges
5. `CrossSampleCNP` - 17 edges
6. `_df()` - 16 edges
7. `featurize_chemical_categoricals()` - 15 edges
8. `configure_logging()` - 15 edges
9. `_make_predictor_stub()` - 15 edges
10. `_attach_scope()` - 14 edges
...
Cohesion: 0.22
Nodes (13): featurize_chemical_categoricals(), Append physicochemical property columns for each chemical categorical.      Pa, P0 regression guard: salt_mw used to be a raw Da value (58-258) that     went ne, buf_mw / stab_mw / surf_mw / exc_mw used to carry raw Da values     (60-8400) st, test_all_chem_categoricals_covered_in_output(), test_drop_original_removes_source_column(), test_known_category_maps_to_its_property_row(), test_missing_column_treated_as_all_none() (+5 more)

### Community 1 - "O-Net CNP Inference (ViscosityPredictorCNP)"
Cohesion: 0.07
Nodes (38): _assert_context_gate(), _check_fold_feature_range(), _effective_n_repeats(), cnp_logo.py =========== CNP side of the Phase 0 leave-one-GROUP-out harness: t, Scales `n_repeats` up for held-out groups smaller than     REFERENCE_N_HELD_FOR, Average calc_metrics (pooled across all shear columns) over     n_repeats rando, Hard guardrail: context (few-shot) must never score worse than     zero-shot by, Guard (b)/(a) from the P0 fix: compare every held-out row's engineered     nume (+30 more)
...
Cohesion: 0.11
Nodes (25): LogRecord, main(), parse_args(), learning_curve_charge_ablation.py ================================== Two-pass va, _init_clean_predictor(), _load_order_ids(), _log_summary(), main() (+17 more)

### Community 25 - "calc_metrics"
Cohesion: 0.06
Nodes (49): _drop_targets(), _importance_for_arm(), main(), parse_args(), _permute_and_score(), DataFrame, charge_feature_importance.py ============================= Permutation feature i, Shared permutation loop for both tiers: `predict_fn(df) -> results_df`     is th (+41 more)
...
Cohesion: 0.10
Nodes (49): _attach_scope(), bin_series(), BinCount, build_gap_ranking(), _class_a_bin_column(), class_a_marginal(), class_a_pair(), class_b_crossings() (+41 more)
...
Cohesion: 0.27
Nodes (16): generate_all(), _hatch_thin_cells(), _label_points_with_collision_avoidance(), plot_p1_logo_support_matrix(), plot_p2_descriptor_occupancy(), plot_p3_concentration_grid(), plot_p4_response_coverage(), plot_p5_gap_pareto() (+8 more)

### Community 46 - "load_and_preprocess"
Cohesion: 0.23
Nodes (15): build_feature_frame(), DataFrame, Run the full row-level feature-engineering pipeline: numeric defaults,     chemi, GUARD SCOPE (follow-on plan Task K): this is the REAL end-to-end guard     again, test_join_fv_charge_columns_matches_after_pipeline_lowercasing(), P0 fix: Salt_mg_mL used to be an unbounded Salt_conc * salt_mw product     that, The core regression test for the bug fix: a formulation with a real     Charge, _sample_row() (+7 more)

### Community 48 - "_drop_blank_rows"
Cohesion: 0.20
Nodes (12): DataFrame, test_coverage.py ================= Task 8 tests for visqai.analysis.coverage.  T, occupied + explicit-NaN bucket == total, for every binned axis this     module d, 500 rows, all the same non-placebo protein -- every class-A axis is     triviall, k clusters of size k each (n = k^2 rows) -- the specific construction     under, _synthetic_equal_clusters_frame(), _synthetic_single_protein_frame(), test_bin_series_never_silently_drops_a_row() (+4 more)
...
Cohesion: 0.22
Nodes (10): describe_property_space(), _keys(), _lookup(), _normalize_category(), DataFrame, categorical_features.py ======================= Rung-1 representation upgrade:, Lowercase, strip, and map empty/nan-likes to 'none'., Map one category value to its ordered descriptor vector.      Substring match (+2 more)

### Community 51 - "objective_cv"
Cohesion: 0.32
Nodes (7): _all_charge_columns(), _all_property_columns(), _get_mw(), protected_feature_indices(), Series, pipeline.py =========== The single row-level feature-engineering pipeline shared, Indices (into num_cols) of features that must never be masked out of     the dec
...
- **Why does `ViscosityPredictorCNP` connect `ViscosityPredictorCNP` to `test_predictor_local_residual.py`, `test_predictor_kernel_corrector.py`, `plot_convergence`, `run_cnp_fold`, `ViscosityPredictorCNP`, `configure_logging`, `calc_metrics`, `CrossSampleCNP`?**
  _High betweenness centrality (0.226) - this node is a cross-community bridge._
- **Why does `build_feature_frame()` connect `load_and_preprocess` to `IBAL Learning-Curve Convergence Analysis`, `O-Net CNP Inference (ViscosityPredictorCNP)`, `Charge Feature Engineering`, `Categorical Feature Engineering`, `ViscosityPredictorCNP`, `objective_cv`, `test_charge_features.py`, `run_cnp_fold`, `calc_metrics`?**
  _High betweenness centrality (0.215) - this node is a cross-community bridge._
- **Why does `load_and_preprocess()` connect `Charge Feature Engineering` to `configure_logging`, `O-Net CNP Inference (ViscosityPredictorCNP)`, `objective_cv`, `load_and_preprocess`?**
