...
measurements, fits an exponential model
    ...
    log₁₀(η) ≈ slope · c_protein + intercept
...
OUTPUT_FILE = "data/processed/augmented_formulation_data_03042026.csv"
...
def detect_flattening(y_linear: np.ndarray) -> tuple[bool, list[int]]:
    ...
    """
    flat_idxs = [
        i for i in range(len(y_linear) - 1) if (y_linear[i] - y_linear[i + 1]) < 0.02 * y_linear[i]
    ]
...
def _collect_anchor_pairs(
    ...
    with_comp = sub[
        sub[type_col].str.lower().str.contains(component_name, na=False) & (sub[conc_col] > 0)
    ]
    ...
    for type_col, conc_col, _ in COMPONENT_DEFS:
        for comp_name in COMPONENTS_PER_COL[type_col]:
            ...
            for ptype in all_proteins:
                pairs = _collect_anchor_pairs(df, ptype, type_col, conc_col, comp_name, visc_cols)
                ...
                if c is not None and c.source not in ("fitted", "single_point"):
                    ...
                    if not donors:
                        donors = [
                            ...
                            if cv.component_name == comp_name and cv.n_pairs >= MIN_ANCHOR_PAIRS
                        ]
            ...
            for comp_name in COMPONENTS_PER_COL[type_col]:
                ...
                for t_conc in target_concs:
                    ...
                    if not high_conc_bases.empty:
                        ...
                        chosen = np.random.choice(len(high_conc_bases), n_high, replace=False)
                        for ci in chosen:
                    ...
                    for row_idx, base_row in selected_bases:
                        ...
                        for draw in range(N_NOISE_DRAWS):
                            ...
                            actual_delta = float(np.clip(predicted_delta + noise, -1.5, 1.5))
...
def generate_protein_conc_interpolation_samples(
    ...
    [ENHANCEMENT-4] Exponential interpolation when ≥ 3 anchor points are available
        (buffer anchor + ≥ 2 measured concentrations).
        The previous log-linear fit assumed a straight line in log-log space
        (power-law), which misrepresents the steeply rising viscosity-concentration
        curve of mAbs at high concentrations.  The exponential model fits
        log₁₀(η) as a linear function of c (mg/mL), giving η ∝ 10^(B·c) —
        consistent with Mooney/Arrhenius-type crowding models.  PCHIP is used
        for within-range interpolation on the (c, log₁₀η) axes, and a least-
        squares exponential slope drives the beyond-c_max_obs extension.
        The 2-anchor fallback also uses the exponential (log-linear-in-c) form.
    ...
    for keys, group in working.groupby(group_cols, dropna=False):
        ...
        # Measured concentrations in linear space — used as x-axis for the
        # exponential model: log₁₀(η) = slope · c + intercept.
        concs_meas = np.array(unique_concs, dtype=float)
        ...
        # Beyond c_max_obs, an exponential continuation (log₁₀η linear in c)
        # is used, which is physically appropriate for the crowded high-conc regime.
        ...
        for vcol in visc_cols:
            ...
            for c in unique_concs:
                ...
                vals = vals[vals > 0]
                log_v_meas.append(float(np.nanmean(np.log10(vals))) if len(vals) > 0 else np.nan)
            ...
            # x-axis is linear protein concentration (mg/mL) for the
            # exponential model: log₁₀(η) = slope · c + intercept.
            if has_buffer_anchor:
                c_fit = np.concatenate([[anchor_conc], concs_meas[valid]])
                lv_fit = np.concatenate([[anchor_log_v], log_v_meas[valid]])
            else:
                c_fit = concs_meas[valid]
            ...
            _, uniq_idx = np.unique(c_fit, return_index=True)
            c_fit = c_fit[uniq_idx]
            lv_fit = lv_fit[uniq_idx]

            if use_pchip and len(c_fit) >= 3:
                # [ENHANCEMENT-6 FIX] Disable PCHIP's built-in extrapolation.
                # PCHIP extrapolates using the slope at the endpoint; that slope
                # is now in (c, log₁₀η) space, so it represents a true exponential
                # continuation — physically appropriate for the crowded high-conc
                # regime.  We switch off PCHIP extrapolate=True and instead drive
                # the beyond-c_max_obs region with an explicit exponential slope
                # (log₁₀η linear in c) fitted to the measured points only.
                # The slope is clamped ≥ 0 (viscosity cannot decrease with conc).
                pchip_interps[vcol] = PchipInterpolator(c_fit, lv_fit, extrapolate=False)
                # Exponential extrapolation slope: fit log₁₀(η) vs c (mg/mL).
                c_meas_valid = concs_meas[~np.isnan(log_v_meas)]
                lv_meas_valid = log_v_meas[~np.isnan(log_v_meas)]
                if len(c_meas_valid) >= 2:
                    A_sl = np.column_stack([c_meas_valid, np.ones_like(c_meas_valid)])
                    ext_slope = float(np.linalg.lstsq(A_sl, lv_meas_valid, rcond=None)[0][0])
                    ext_slope = max(ext_slope, 0.0)  # clamp: visc cannot fall with conc
                else:
                    ext_slope = 0.0
                # Anchor the exponential extension to the last measured point
                # in linear-c space.
                c_ext_anchor = float(unique_concs[-1])
                lv_ext_anchor = float(
                    np.nanmean(
                        np.log10(
                            np.maximum(
                ...
                pchip_interps[vcol + "__ext_anchor_c"] = c_ext_anchor
                pchip_interps[vcol + "__ext_anchor_lv"] = lv_ext_anchor
            else:
                # Exponential fallback: log₁₀(η) = slope·c + intercept.
                # Used when PCHIP cannot be applied (< 3 unique anchor points).
                if len(c_fit) >= 2:
                    A = np.column_stack([c_fit, np.ones_like(c_fit)])
        ...
        new_log_concs: list[float] = [
            ...
            if all(abs(10.0**lc - ec) / (ec + 1e-9) > exclusion_frac for ec in unique_concs)
        ]
        ...
        for i, log_c in enumerate(new_log_concs):
            ...
            for vcol in visc_cols:
                if vcol in pchip_interps:
                    # Retrieve exponential extrapolation parameters.
                    # c_ext_anchor is in mg/mL (linear); ext_slope is
                    # Δlog₁₀(η) per mg/mL.
                    c_ext_anchor = pchip_interps[vcol + "__ext_anchor_c"]
                    lv_ext_anchor = pchip_interps[vcol + "__ext_anchor_lv"]
                    ext_slope = pchip_interps[vcol + "__ext_slope"]
                    if new_c <= c_ext_anchor:
                        # Within the measured range: query PCHIP with linear c.
                        log_v_pred = float(pchip_interps[vcol](new_c))
                    else:
                        # Exponential extrapolation: log₁₀(η) = lv_anchor + slope·Δc.
                        log_v_pred = lv_ext_anchor + ext_slope * (new_c - c_ext_anchor)
                elif vcol in ll_fits:
                    # Exponential fallback: log₁₀(η) = slope·c + intercept.
                    sl, ic = ll_fits[vcol]
                    log_v_pred = sl * new_c + ic
                else:
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    print("\n[Pass 3] Interpolating protein concentration curves (exponential fit)...")
    ...
    print(f"  {'Class':<30} {'core':>5} {'p1':>5} {'p2':>5} {'p3':>5} {'total':>6}  core_flat")
    print("  " + "-" * 72)
    for p in sorted(type_core_counts):
