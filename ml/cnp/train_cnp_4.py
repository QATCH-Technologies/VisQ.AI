...
def load_and_preprocess(csv_path, save_dir=None):
    ...
    return samples, X_matrix.shape[1], physics_scaler
...
def compute_viscosity_weights(
    ...
    unweighted baseline. Without normalisation, upweighting inflates the loss
    magnitude and destabilises the relative contributions of lambda_utility,
    lambda_triplet, etc.

    Args:
        qy_scaled   : Tensor [..., 1] of StandardScaler-normalised log10-viscosity.
        visc_mean   : physics_scaler.mean_[1]  (log10-space mean).
        visc_scale  : physics_scaler.scale_[1] (log10-space std-dev).
        threshold   : log10(viscosity) pivot point.  Default 2.0 = 100 cP.
        max_weight  : Peak multiplier at the high-viscosity extreme.
                      Default 4.0 — before normalisation; effective peak after
                      normalisation will be lower depending on data distribution.
        steepness   : Sigmoid slope around the threshold.  Default 3.0 ≈ 1-decade
                      transition region.

    Returns:
        Tensor of same shape as qy_scaled, normalised so mean == 1.0.
    """
    log_visc = qy_scaled * visc_scale + visc_mean
    raw_w = 1.0 + (max_weight - 1.0) * torch.sigmoid(steepness * (log_visc - threshold))
    # Normalise so mean weight == 1.0 — this is the key fix that prevents
    # weight inflation from changing the effective magnitude of mse_loss and
    # breaking the calibrated balance with utility / triplet / norm losses.
    normalised_w = raw_w / (raw_w.mean() + 1e-8)
    return normalised_w.detach()  # weights are not learnable — stop gradient
...
def train_epoch(
    ...
    lambda_norm=0.05,
    norm_target=5.0,
    # [FIX-VISCWEIGHT] Per-point viscosity-regime upweighting applied ONLY to
    # the primary mse_loss. The utility loss comparisons (mse_null /
    # mse_ctx_unmasked) intentionally use flat F.mse_loss — they measure
    # context informativeness and must stay on the same scale as each other to
    # avoid destabilising the lambda_utility balance.
    physics_scaler=None,
    visc_threshold=2.0,  # log10(100 cP) — pivot between low/high viscosity
    visc_max_weight=4.0,  # peak weight before mean-normalisation
):
    # Pre-extract scaler parameters once per epoch (avoids repeated attribute
    # lookups in the hot loop). Falls back gracefully when no scaler supplied.
    ...
    for _ in range(iterations):
        ...
        # [FIX-VISCWEIGHT] Weighted MSE for the primary loss only.
        # Weights are mean-normalised so the loss scale is unchanged vs the
        # unweighted baseline.  High-viscosity points receive stronger gradient
        # signal without inflating the total loss magnitude.
        if _visc_mean is not None:
            w_A = compute_viscosity_weights(
                qy_A,
                _visc_mean,
                _visc_scale,
                threshold=visc_threshold,
                max_weight=visc_max_weight,
            )
            mse_loss = (w_A * (pred_A - qy_A) ** 2).mean()
        else:
            mse_loss = F.mse_loss(pred_A, qy_A)

        # Unweighted MSE kept for per-group EMA difficulty tracking so that
        # system stays on a consistent scale independent of viscosity weighting.
        mse_loss_unweighted = F.mse_loss(pred_A, qy_A).detach()

        # ---- Context utility loss (intentionally FLAT / unweighted) ----
        # This loss compares prediction quality with vs without context.
        # Both sides must use the same loss function or the comparison is
        # meaningless.  Applying viscosity weights here would scale up the
        # effective lambda_utility for high-viscosity batches and cause the
        # utility term to dominate, which was the root cause of the prior
        # constant-0.5-cP collapse.
        with torch.no_grad():
...
def objective_cv(trial, samples, static_dim, device, physics_scaler=None):
...
if __name__ == "__main__":
    ...
    samples, static_dim, physics_scaler = load_and_preprocess(data, save_dir=out)
    print(
    ...
    if trials > 0:
        ...
        study.optimize(
            lambda t: objective_cv(t, samples, static_dim, device, physics_scaler),
            n_trials=trials,
        )
