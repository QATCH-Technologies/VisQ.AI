    ...
    for _ in range(iterations):
        ...
        def build_batch(sample_indices):
            ...
            for i in sample_indices:
                s = task_samples[i]
                # Combine points for efficiency
                shear_list.append(s["points"][:, [0]])
                y_list.append(s["points"][:, [1]])
                # Repeat static for every point
            ...
            # Context input is [Shear, Visc, Static]
            # Context points need both shear and visc (y)
            # We must reconstruct the combined tensor
            # s["points"] contains [Shear, Visc]
            # We need to concat all s["points"] + s["static"]
            ctx_items = []
            for i in sample_indices:
                s = task_samples[i]
                stat_expanded = s["static"].unsqueeze(0).repeat(s["points"].shape[0], 1)
                ctx_items.append(torch.cat([s["points"], stat_expanded], dim=1))

            ctx_tensor = torch.cat(ctx_items, dim=0).unsqueeze(0).to(device)

            return ctx_tensor, all_shear, all_stat, all_y
        ...
        if torch.isnan(loss):
            # FIX: Skip optimization if loss is NaN to prevent crash, but print warning
...
def validate(model, samples, device):
    ...
    if count == 0:
        return float("inf")  # Return high error if validation failed completely
...
if __name__ == "__main__":
    data = "data/raw/formulation_data_02122026.csv"
