...
class CarreauYasudaCNP(nn.Module):
    def __init__(self, static_dim, hidden_dim=128, latent_dim=128, dropout=0.0):
        ...
        # The decoder NO LONGER takes shear rate as an input.
        # It maps the formulation (static + latent r) directly to the 5 CY parameters.
        self.param_net = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 5),  # Outputs raw values for the 5 parameters
        )

        # Buffers to hold the scaler means/stds for unscaling inside the forward pass
    ...
    def set_scaler(self, physics_scaler):
        """Must be called after initialization to pass the scaler variables to the model."""
    ...
    def forward(self, context_tensor, query_shear, query_static):
        ...
        # 1. Predict raw parameters from formulation
        param_input = torch.cat([query_static, r_expanded], dim=-1)
        raw_params = self.param_net(param_input)

        # 2. Constrain parameters to physical realities
        # eta_0: log-space for positivity
        eta_0 = torch.exp(raw_params[..., 0:1])

        # eta_inf: strictly a fraction of eta_0
        eta_inf = eta_0 * torch.sigmoid(raw_params[..., 1:2])

        # lambda: log-space for positivity
        lam = torch.exp(raw_params[..., 2:3])

        # a: softplus for > 0
        a = F.softplus(raw_params[..., 3:4]) + 1e-3

        # n: sigmoid to strictly enforce shear-thinning (0 to 1)
        n = torch.sigmoid(raw_params[..., 4:5])

        # 3. Unscale shear rate z-scores back to raw physical values
        log10_shear_raw = query_shear * self.shear_std + self.shear_mean
        shear_raw = 10.0**log10_shear_raw

        # 4. Apply the Carreau-Yasuda equation
        bracket = 1.0 + (lam * shear_raw) ** a
        exponent = (n - 1.0) / a
        visc_raw = eta_inf + (eta_0 - eta_inf) * (bracket**exponent)

        # 5. Scale the resulting viscosity back to z-scores to match training targets
        log10_visc_raw = torch.log10(visc_raw + 1e-9)
        visc_scaled = (log10_visc_raw - self.visc_mean) / self.visc_std

        return visc_scaled

    # Keep encode_memory and decode_from_memory as they are in your script,
    # just update decode_from_memory to use the same logic as the forward pass above.
...
if __name__ == "__main__":
    ...
    final_model = CarreauYasudaCNP(
        static_dim,
        hidden_dim=best_params["hidden_dim"],
        latent_dim=best_params["latent_dim"],
        dropout=best_params["dropout"],
    ).to(device)
    final_model.set_scaler(physics_scaler)
    optimizer = torch.optim.AdamW(
